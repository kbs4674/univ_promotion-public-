class Ability
  ...
  def initialize(user)
    ...
    if user.has_role? :admin #어드민 권한
      ...
      cannot [:new, :create, :edit, :update, :destroy], Univ, user_id: user.id
      cannot [:show, :new, :create, :edit, :update, :destroy], Promotion, user_id: user.id
