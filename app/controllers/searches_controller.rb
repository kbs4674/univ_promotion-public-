class SearchesController < ApplicationController
  ...
  def univ_search
    @searches = Univ.where("univ_name_kor like ?", "%#{params[:search_univ]}%").order("created_at DESC").page(params[:page]).per(10).or(Univ.where("univ_name_eng like ?", "%#{params[:search_univ]}%").order("created_at DESC").page(params[:page]).per(10))
  end
end